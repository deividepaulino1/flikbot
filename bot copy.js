...
client.on("message", async (message) => {
  ...
  if (comando === "iniciando") {
    ...
    flood = setInterval(() => {
      ...
      // Verifica se Existe Acess Violation na máquina do luciano
      if (fs.existsSync(`${path}/avluciano.txt`)) {
          client.channels.cache
            .get("984505160057384990")
            .send("Erros reportados na máquina do luciano", {
              files: [`${path}/avluciano.txt`],
            });

          setTimeout(() => {
            arquivo.deletarLogZip('/avluciano.txt')
            console.log("Erros reportados pelo luciano");
          }, 5000);
        } 
      ...
      if (fs.existsSync(`${path}/avFabio.txt`)) {
          ...
          client.channels.cache
            .get("984505160057384990")
            .send("Erros reportados na máquina do Fábio", {
      ...
      if (fs.existsSync(`${path}/avServidor.txt`)) {
          client.channels.cache
            .get("984505160057384990")
            .send("Erros reportados na máquina Servidor", {
